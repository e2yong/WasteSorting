...
//        CompoundButton.OnCheckedChangeListener,
  ...
  protected ListView resultView, deviceView, modelView;
  ...
  protected void onCreate(final Bundle savedInstanceState) {
    ...
    // 슬라이드뷰 내부 객체 탐지 결과 목록
    resultView = findViewById(R.id.result_list);
    resultStrings.add(empty);
    resultView.setChoiceMode(ListView.CHOICE_MODE_SINGLE);
    ArrayAdapter<String> resultAdapter =
            new ArrayAdapter<>(
                    CameraActivity.this, R.layout.resultview_row, R.id.resultview_row_text, resultStrings);
    resultView.setAdapter(resultAdapter);
    resultView.setOnItemClickListener(
            new AdapterView.OnItemClickListener() {
              @Override
              public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                // 선택한 쓰레기의 대분류
                String selectedItem = resultStrings.get(i);

                if (!selectedItem.equals(empty)) {
                  // 소분류 페이지로
                  Intent intent = new Intent(CameraActivity.this, SortActivity.class);
                  intent.putExtra("large", selectedItem);
                  startActivity(intent);
                }
              }
            });

    // 슬라이드뷰 내부 디바이스 목록 - 사용X, 객체 탐지에 사용
    ...
    // 슬라이드뷰 내부 모델 목록 관련 - 표시X, 객체 탐지에 사용
  ...
  // 주어진 경로에 있는 모델 파일(.tflite)들을 리스트에 저장
